...
CREATE POLICY "Activities publicly viewable" ON public.activities FOR SELECT TO anon USING (true); GRANT SELECT ON public.activities TO anon;ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS duration_hours integer NOT NULL DEFAULT 2 CHECK (duration_hours > 0 AND duration_hours <= 24);DROP POLICY IF EXISTS "RSVPs viewable by authenticated" ON public.rsvps;
